...
#define DEC_ENON			( 0)				// Executing right
#define DEC_EINI			(-1)				// Error during init.
#define DEC_EDEC			(-2)				// Error during image decoding

extern uint8_t (*dst_image)[120][160][3];
extern float scale;
extern int32_t zero_point;
...
//void decodeMyImage(uint8_t* pdata, int iDataSize, int x, int y, int iOptions, JPEG_DRAW_CALLBACK *pfnDraw, int);
int32_t decodeMyImage(uint8_t* pdata, int iDataSize, int x, int y, int iOptions, JPEG_DRAW_CALLBACK *pfnDraw, int);
